...
export default function TravelDestinations() {
  ...
  const refreshOne = async (id) => {
    try {
      ...
      if (isOk(data) && data?.result) {
        ...
        setForm((f) => ({ ...f, coverImageUrl: fresh.coverImageUrl || f.coverImageUrl }));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const data = await response.json();
      if (!isOk(data)) throw new Error(data?.message || "Failed to save");
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      const data = await response.json();
      if (!isOk(data)) throw new Error(data?.message || "Upload failed");
  ...
  const handleImageSave = async () => {
    ...
    try {
      ...
      const data = await response.json();
      if (!isOk(data)) throw new Error(data?.message || "Failed to update image");
  ...
  const handleImageDelete = async (img) => {
    ...
    try {
      ...
      const data = await response.json();
      if (!isOk(data)) throw new Error(data?.message || "Failed to delete image");
  ...
  const moveImage = async (idx, dir) => {
    ...
    try {
      ...
      const data = await response.json();
      if (!isOk(data)) throw new Error(data?.message || "Failed to reorder");
      await refreshOne(editingId);
    } catch (err) {
  ...
  const setAsCover = async (img) => {
    try {
      ...
      const data = await response.json();
      if (!isOk(data)) throw new Error(data?.message || "Failed to set cover");
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12}>
                  <TextField
                    ...
                    multiline
                    rows={2}
                    helperText="Shown on the listing card."
                  />
                </Grid>
                <Grid item xs={12}>
                  <TextField
                    ...
                    multiline
                    rows={2}
                    helperText="Editorial quote shown in the overview panel. Falls back to short description if empty."
                  />
                </Grid>
                <Grid item xs={12}>
                  <TextField
                ...
                {/* Cover image URL fallback (auto-resolved if empty) */}
                <Grid item xs={12}>
                  <TextField
                    ...
                    fullWidth
                    size="small"
                    placeholder="Leave empty to auto-use the first slideshow image"
                  />
                </Grid>

                {/* Custom plan calculator: per-person entry fee + coordinates */}
                ...
                <Grid item xs={12} md={4}>
                  <TextField
                    ...
                    helperText="Used by the trip distance calculator."
                  />
                </Grid>
                <Grid item xs={12} md={4}>
                  <TextField
